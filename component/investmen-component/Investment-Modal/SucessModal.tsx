...
export default function SuccessModal(props: any) { 
    ...
    return (
        <div style={{borderRadius: '16px'}} className='w-auto h-auto flex flex-col justify-center items-center bg-white pt-8 pb-6 px-12' >
            ...
            <button onClick={()=> {props.image === 'dollar' ?  props.close(false):props.end === 'loan' ?  props.close(-1):  props.close(0)}} style={{backgroundColor: '#002343'}} className=' w-full text-white rounded py-3 mt-4 font-Montserrat-Bold text-xs text-center' >Continue</button>
        </div>
    );
}
